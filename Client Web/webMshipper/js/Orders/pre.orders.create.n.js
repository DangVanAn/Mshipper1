angular.module('mShipperApp')
    .component('orderCreateN', {
        templateUrl: './Orders/Pre.Create.n.html',
        controller: function DsDonHangController($rootScope, $scope, $http, $filter, ngDialog, $location, $timeout, $parse, modalOrderCreate, NgMap, NgTableParams) {
            ...
            $scope.GetImport = function () {
                ...
                for (var i = 0; i < json.Sheet1.length; i++) {
                    ...
                    //Lấy tổng số tấn của tất cả đơn hàng.
                    sumTon += Number(data._ton);
            ...
            $scope.saveOrders = function () {
                ...
                if (listOrderHandling1 == 0) {
                    ...
                    //Tạo một preorder_list
                    createPreOrderList();
                }
            };

            function createPreOrderList() {

                var code_data = (new Date().getTime()).toString();
                var data = {
                    _code_file : code_data,
                    _create_user : $rootScope.globals.currentUser.id,
                    _create_user_name : $rootScope.globals.currentUser.name,
                    _number : listOrderHandling1.length,
                    _ton : sumTon,
                    _status : 'ok',
                    _is_enable : true
                };

                $urll = 'http://localhost:9999/preorderslist/add';
                ...
                $http({
                    ...
                    if(response.data === 'error!')
                    {
                        ...
                        //Sao lưu đơn hàng. với
                        createPreOrders(code_data);
                    }
            ...
            function createPreOrders(code_file) {
                for(var i = 0; i < listOrderHandling1.length; i++){
                    ...
                    listOrderHandling1[i]._code_file = code_file;
                }

                $urll = 'http://localhost:9999/preorders/addsnew';
